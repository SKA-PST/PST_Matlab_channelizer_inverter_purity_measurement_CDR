% Script to test oversampled PFB inversion via FFT for SKA_Mid channelization
% and measure the resulting spectral leakage
...
% PFB parameters
N = 4096;  % number of output channels
OS_Nu = 8;  % OS factor numerator
OS_De = 7;  % OS factor denominator
...
% 4096 channels, 8/7 OS, 100,353 taps (24.5 per channel + 1), 16-bit quantization
% h is unquantized coefficients, hQ is 16-bit quantized
load('NRC_100353_MidFilterCoefficients.mat');
...
if(equaliseRipple)
    ...
    [H0,W] = freqz (hQ, 1, N*passbandLength);
...
%     subplot(212); plot((1:blockLength),phase(spectrum(1:blockLength,1))); box on; grid on; title('sig Phase'); xlabel('frequency');
    ...
    output = polyphase_analysis(transpose(inputSig),hQ,N,PFBarms);
...
figure;
plot((1:blockLength),errorTFdB(1:blockLength,1)); box on; grid on; axis([1 blockLength -200 -100]); title('Power Error vs Frequency'); xlabel('Frequency Bin'); ylabel('Power (dB)');
